...
declare t text;
begin
  foreach t in array array['users','categories','vendors','fx_rates','recurring','expenses','expense_shares','recurring_shares','settlements']
  loop
...
-- fx_rates: also allow the anon/service path to read (cron uses service role which bypasses RLS,
-- but a read policy keeps client-side "current rate" display working for signed-in users only).
